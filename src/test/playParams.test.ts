...
describe('parsePlayParams', () => {
  it('returns isValid=true when all params are valid', () => {
    const result = parsePlayParams({ decks: 'emergencies', age: '10+', mode: 'discussion' });
  ...
  it('accepts multiple comma-separated decks', () => {
    const result = parsePlayParams({ decks: 'emergencies,bullying', age: '7-9', mode: 'quiz' });
    expect(result.isValid).toBe(true);
    expect(result.params.deckIds).toEqual(['emergencies', 'bullying']);
  });

  it('filters out invalid deck slugs', () => {
    const result = parsePlayParams({ decks: 'emergencies,fake-deck', age: '10+', mode: 'discussion' });
    expect(result.isValid).toBe(true);
    expect(result.params.deckIds).toEqual(['emergencies']);
  });

  it('returns isValid=false when decks param is missing', () => {
    const result = parsePlayParams({ decks: null, age: '7-9', mode: 'discussion' });
    expect(result.isValid).toBe(false);
    expect(result.params.deckIds).toEqual([]);
  });

  it('returns isValid=false when all deck slugs are invalid', () => {
    const result = parsePlayParams({ decks: 'nope,bad', age: '7-9', mode: 'quiz' });
    expect(result.isValid).toBe(false);
  });
  ...
  it('falls back age to 7-9 when invalid', () => {
    const result = parsePlayParams({ decks: 'emergencies', age: 'bad', mode: 'discussion' });
    expect(result.params.age).toBe('7-9');
  });

  it('returns isValid=false when mode is invalid', () => {
    const result = parsePlayParams({ decks: 'emergencies', age: '10+', mode: 'battle' });
    expect(result.isValid).toBe(false);
  });

  it('falls back mode to discussion when invalid', () => {
    const result = parsePlayParams({ decks: 'emergencies', age: '10+', mode: '' });
    expect(result.params.mode).toBe('discussion');
  });
...
describe('buildPlayUrl', () => {
  it('constructs a valid URL', () => {
    ...
    expect(url).toBe('https://example.com/play?decks=emergencies&age=10%2B&mode=discussion');
  });
...
describe('smoke: /play?decks=emergencies&age=10+&mode=discussion', () => {
